...
def main():
    ...
    ori_err = rawdata[:, 21].astype(float)
    est_q = [np.quaternion(*q) for q in rawdata[:, 22:26].astype(float)]
    dist_err = np.empty((info.shape[0], 0))  # TODO: add dist_err to evaluate.py, add code to plot and print stats below
    lat_err = np.empty((info.shape[0], 0))   # TODO: add lat_err to evaluate.py, add code to plot and print stats below
